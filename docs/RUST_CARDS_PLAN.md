...
> **Оновлення — цей документ історичний.** Python-рушія (`hs2/`) більше
> немає в дереві, тож усі посилання на `hs2/impls.py`,
> `hs2/build_data.py` і `scripts/build_cards.py` читайте як опис того,
> звідки прийшли дані й семантика карт, а не як робочі команди. Корпуси
> лежать у `data/`, гаунтлет — у `data/gauntlet_standard.json`, і числа
> покриття нижче застаріли: актуальні дає `tavernsim coverage`.
...
`tavernsim implemented` (на дату документа — 198 назв; 41 карта реалізована
після зрізу). Файл відтоді видалено; актуальний список дає сама команда.

## 1. Стан на 2026-08-25 (виміряно тоді)

Числа нижче — зріз на дату документа, а не сьогоднішні. Актуальні дає
`tavernsim coverage`; для порівняння, Standard відтоді пройшов
198 → 885 з 1210.
